...
Shader "URPGlitch/AnalogBorder"
{
    Properties
    {
        _EdgeStart ("Edge Start", Range(0, 1)) = 0.35
        _EdgeEnd ("Edge End", Range(0, 1)) = 0.5
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Fragment(Varyings i) : SV_Target
            {
                ...
                half4 glitch = half4(src1.r, src2.g, src1.b, 1);

                // Only show edges
                half4 original = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearRepeat, i.texcoord);
                float2 center = float2(0.5, 0.5);
                float dist = distance(i.texcoord, center);
                float mask = smoothstep(_EdgeStart, _EdgeEnd, dist);

                return lerp(original, glitch, mask);
            }
            ENDHLSL
        }
    }
